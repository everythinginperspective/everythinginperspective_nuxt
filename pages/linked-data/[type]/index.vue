<template>
  <main class="max-w-7xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
    <div class="mb-12 border-b-2 border-accent pb-6">
      ...
      <h1 class="font-serif text-5xl font-bold mb-2 capitalize">{{ contentType?.plural }}</h1>
      <p class="text-muted">Graph-based view of all {{ contentType?.plural }} with relationship connections</p>
    </div>
...
const contentType = await getContentTypeByPlural(type)
if (!contentType) {
...
useSeoMeta({
  ...
  description: `Graph-based view of all ${contentType.plural} with relationship connections`,
  ogTitle: `${contentType.plural} - Linked Data | Everything in Perspective`
})
</script>
